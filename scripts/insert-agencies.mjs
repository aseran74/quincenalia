// Script para insertar agencias inmobiliarias en Supabase
...
const supabaseKey = envVars.SUPABASE_SERVICE_ROLE_KEY || envVars.VITE_SUPABASE_ANON_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.VITE_SUPABASE_ANON_KEY;
...
async function insertAgencies() {
  ...
  for (let i = 0; i < agencies.length; i += batchSize) {
    ...
    if (error) {
      // Si el error es por duplicado, lo contamos como skipped
      if (error.code === '23505' || error.message.includes('duplicate') || error.message.includes('unique')) {
        console.log(`  ⚠️  Algunas agencias ya existen (duplicados), continuando...`);
        skipped += batch.length;
      } else {
        console.error(`  ❌ Error en lote ${batchNum}:`, error.message);
        errors++;
        // Intentar insertar una por una para identificar cuáles fallan
        for (const agency of batch) {
          const { error: singleError } = await supabase
            .from('real_estate_agencies')
            .insert(agency)
            .select();
          if (singleError && !singleError.message.includes('duplicate') && !singleError.message.includes('unique')) {
